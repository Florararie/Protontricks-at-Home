...
from Classes.Steam.Steam import SteamUser
from Classes.Actions import Actions
from Classes.GUI.ActionDialog import ActionDialog
from Classes.GUI.HighlightDelegate import HighlightDelegate
from Classes.GUI.GameListModel import GameListModel, GameSortFilterProxyModel
...
class MainWindow(QWidget):
    def __init__(self, data, steam_root, steam_id, steam_installation, user_switcher_callback: Optional[Callable[[str, str], None]] = None):
        super().__init__()
    ...
    def _populate_user_dropdown(self):
        ...
        self.user_dropdown.setEnabled(True)

        for user_id, user_name in users:
            avatar_icon = self._get_user_avatar(user_id)

            index = self.user_dropdown.count()
            self.user_dropdown.addItem(avatar_icon, user_name)
            self.user_dropdown.setItemData(index, user_id, Qt.UserRole)
    ...
    def on_user_changed(self, index):
        if not self.user_switcher_callback or index < 0:
            return

        new_user_id = self.user_dropdown.itemData(index, Qt.UserRole)
        new_user_name = self.user_dropdown.itemText(index)

        if not new_user_id or new_user_id == self.steam_id:
            return

        QApplication.setOverrideCursor(Qt.WaitCursor)
        try:
            self.user_switcher_callback(new_user_id, new_user_name)
    ...
    def show_context_menu(self, pos):
        ...
        menu = QMenu(self)
        self._add_menu_action(menu, "Launch", lambda: Actions.launch_game(self.steam_installation, game["type"], game["appid"]), is_owner)
    ...
    def _load_assetcache(self):
        ...
        try:
            ...
            logger.error(f"Failed to load assetcache: {e}")
